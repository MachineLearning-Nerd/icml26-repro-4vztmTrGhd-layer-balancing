...
2. **Closed-form gradients and losses — verified.** Exact whitened-data gradients
   match PyTorch autograd in float64, the one-step loss matches an independent
   compact polynomial expansion, and direct Haar-matrix signal-only updates agree
   statistically with the paper's expected-loss formulas.
